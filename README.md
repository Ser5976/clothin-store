...
1.  Клонируйте проект на свой компьютер
    - git clone https://github.com/Ser5976/clothin-store.git
...
В проекте предусмотрена обработка ошибок. В зависимости от способа работы с данными, применяются разные подходы:
      ...
      1. Когда используется fetch() на серверной стороне,  ошибки обрабатываются вручную через  проверку res.ok
         Если res.ok === false, реализуются два подхода:
          ...
          - throw new Error() - если выбрасывается ошибка, срабатывает файл error.tsx ( который находится в этой папке 3), и отображается глобальный fallback UI для ошибок страницы. Этот подход позволяет централизованно обрабатывать ошибки на уровне страницы
...
В проекте предусмотрена обработка состояния загрузки. В зависимости от способа работы с данными, применяются разные подходы:
      ...
      2. Когда используется TanStack Query. TanStack Query автоматически предоставляет состояние  загрузки. Это состояния легко  обрабатываются через переменные  isLoading
...
- SHOP: New arrivals — ссылка на страницу с новыми поступлениями,Trending now — популярные товары, Sales — товары со скидками
...
Кастомные формы регистрации и входа:
...
Поиск товаров реализован через поле ввода, расположенное в header. Пользователь может найти нужный товар по ключевым словам. Поиск выполняется на стороне сервера и охватывает следующие характеристики товара:
