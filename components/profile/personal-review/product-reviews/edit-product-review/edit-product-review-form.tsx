...
import { updateProductReviewAction } from '@/server-action/user/update-product-review-action';
import {
  EditReviewFormType,
  EditReviewFormValidator,
} from '@/validators/edit-review-form-validator';

type EditProductReviewFormProps = {
  content: string;
  id: string;
  setIsOpen: React.Dispatch<React.SetStateAction<boolean>>;
};

export const EditProductReviewForm: FC<EditProductReviewFormProps> = ({
  setIsOpen,
  content,
  id,
}) => {
  ...
  const onSubmit = (data: { content: string }) => {
    ...
    setIsLoading(true);
    updateProductReviewAction(newContent)
      .then((res) => {
        setIsLoading(false);
        if (res.success) {
